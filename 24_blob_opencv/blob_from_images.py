...
text = f"Label {classes[idx]}, {preds[0][idx] * 100:.2f}%"
...
# loop over the input images (excluding the first one sice we
...
print(f"Second Blob: {blob.shape}")
...
for (i, p) in enumerate(imagePaths[1:]):
    ...
    text = f"Label: {classes[idx]}, {preds[i][idx] * 100:.2f}%"
    cv2.putText(image, text, (5, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
